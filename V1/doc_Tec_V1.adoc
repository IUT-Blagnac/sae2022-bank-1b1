...
La classe `EmployeEditorPane.java` et la classe controlleur `EmployeEditorPaneController.java` affichent la fenêtre de modification d'un employé et remplit les champs préexistants grâce à l'employé sélectionné graphiquement, récupéré par la `ListView`. +
